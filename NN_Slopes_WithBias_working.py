...
class Trainer(object):
    def __init__(self,N):
        ...
        self.Wbinori=np.zeros(self.N.Wbin.shape)
        self.Wboutori = np.zeros(self.N.Wbout.shape)
        ...
        #while sum(abs(Y-self.N.yHat)) >0.002:
            for p1 in range(number_of_inputs):
                for p2 in range(number_of_neurons):
                    for p3 in range(number_of_output_neurons):
                        ...
                        self.Wbinori[p1,p2]=self.N.Wbin[p1,p2]
                        self.N.Wbin[p1,p2]=self.N.Wbin[p1,p2] + increment_for_slope
                        self.N.yHat_calculator(X)
                        self.Jnew = self.N.cost_calculator(Y)
                        self.slope = self.Jnew - self.Jold
                        self.N.Wbin[p1,p2] = self.Wbinori[p1,p2] - (self.slope)*speed
                        self.N.yHat_calculator(X)
                        self.Jold = self.N.cost_calculator(Y)
                        
          
                        self.Wboutori[p3,0]=self.N.Wbout[p3,0]
                        self.N.Wbout[p3,0]=self.N.Wbout[p3,0]+increment_for_slope
                        self.N.yHat_calculator(X)
                        self.Jnew = self.N.cost_calculator(Y)
                        self.slope=self.Jnew-self.Jold
                        self.N.Wbout[p3,0] = self.Wboutori[p3,0] - (self.slope) * speed
                        self.N.yHat_calculator(X)
                        self.Jold = self.N.cost_calculator(Y)
